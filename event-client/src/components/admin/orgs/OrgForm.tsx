...
const OrgForm = ({ mode, initialValues, currentAdmin }: Props) => {
  ...
  useEffect(() => {
    if (mode === "edit" && !isInitialized && initialValues && currentAdmin) {
      if (nameRef.current) nameRef.current.value = initialValues.name;
      if (descripRef.current)
        descripRef.current.value = initialValues.description;
      ...
      setIsInitialized(true); // key to set
    }
  }, [initialValues, currentAdmin, mode, isInitialized, nameRef, descripRef]);
  ...
  const addOrgHandler = (e: React.FormEvent) => {
    ...
    if (mode == "create") {
      ...
      formData.append("orgID", String(initialValues?.id));

      if (currentAdmin?.id != adminInfo.id) {
        formData.append("oldAdminID", String(currentAdmin?.id));
        axiosClient
          .post("/org/update", formData)
          .then((response) => {
            if (response.data == "201") {
              setLoading(false);
              navigate("/admin/orgs/list");
